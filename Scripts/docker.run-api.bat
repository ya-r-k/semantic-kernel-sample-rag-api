...
for %%C in (mongodb neo4j qdrant ollama) do docker network connect samplerag-net %%C 2>nul
...
docker build -t sampleragapi ../SampleRag.API

REM Запустить контейнер API
docker run -d --name sampleragapi --network samplerag-net -p 5000:80 ^
  -e MongoSettings__ConnectionString=mongodb://mongodb:27017 ^
