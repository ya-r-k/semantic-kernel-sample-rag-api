    ...
    Kernel kernel) : IDataGenerator
{
    ...
    public async IAsyncEnumerable<MessagePart> GenerateStreamingData(IEnumerable<Message> messages, [EnumeratorCancellation] CancellationToken ct = default)
    {
        var chat = messages.Adapt<ChatHistory>();
        var chatCompletion = kernel.GetRequiredService<IChatCompletionService>();
